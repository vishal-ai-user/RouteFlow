"""AEGIS database repositories — Typed database access methods.
...
from aegis.persistence.db import (
    decrypt_val,
    encrypt_val,
    get_db_connection,
    get_encryption_key,
...
class LogRepository:
    ...
    def _update_request_status_sync(
        request_id: str, status_code: int, latency_ms: int, db_path: str | None
    ) -> None:
        with get_db_connection(db_path) as conn:
            conn.execute(
                "UPDATE request_logs SET status_code = ?, latency_ms = ? WHERE request_id = ?",
                (status_code, latency_ms, request_id),
            )

    async def update_request_status(
        self, request_id: str, status_code: int, latency_ms: int, db_path: str | None = None
    ) -> None:
        """Update HTTP status code and latency of a request."""
        await asyncio.to_thread(
            self._update_request_status_sync, request_id, status_code, latency_ms, db_path
        )
    ...
    def _get_request_logs_sync(limit: int, db_path: str | None) -> list[RequestLog]:
        with get_db_connection(db_path) as conn:
            ...
            cursor.execute(
                """
                SELECT id, request_id, model, stream, status_code, latency_ms, created_at
                FROM request_logs
                ORDER BY id DESC
                LIMIT ?
                """,
