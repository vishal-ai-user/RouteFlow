"""RouteFlow provider pool — In-memory registry and health tracking for pool members.
...
def get_global_pool() -> ProviderPool:
    ...
    # Loop from 1 to 10 looking for ROUTEFLOW_NVIDIA_{i}_API_KEY
    for i in range(1, 11):
        api_key_env = f"ROUTEFLOW_NVIDIA_{i}_API_KEY"
        label_env = f"ROUTEFLOW_NVIDIA_{i}_LABEL"
        base_url_env = f"ROUTEFLOW_NVIDIA_{i}_BASE_URL"
    ...
    # Fallback: check for ROUTEFLOW_NVIDIA_API_KEY if no dynamic indexed accounts are found
    if not pool._members:
        single_key = os.environ.get("ROUTEFLOW_NVIDIA_API_KEY")
        if single_key:
            base_url = (
                os.environ.get("ROUTEFLOW_NVIDIA_BASE_URL") or "https://integrate.api.nvidia.com/v1"
            )
            provider = NvidiaProvider(
                name="default-nvidia",
                api_key=single_key,
    ...
    try:
        ...
        from routeflow.persistence.db import decrypt_val, get_db_connection, get_encryption_key
