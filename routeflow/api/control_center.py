"""RouteFlow Control Center API — Backend routes for administrative control and monitoring.
...
def _get_uptime_seconds() -> float:
    try:
        from routeflow.main import START_TIME
...
@router.delete("/api/logs", dependencies=[Depends(require_auth)])
async def clear_all_logs() -> dict[str, bool]:
    """Delete all request, response, error, and usage logs from the database."""
    log_repo = LogRepository()
    await log_repo.clear_all_logs()
    return {"ok": True}
