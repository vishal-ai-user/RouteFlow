"""RouteFlow settings — Environment-driven configuration using Pydantic Settings.

All settings are loaded from environment variables prefixed with ROUTEFLOW_.
...
class AegisSettings(BaseSettings):
    """Core RouteFlow configuration loaded from environment variables.

    Each field maps to an ROUTEFLOW_<FIELD_NAME> environment variable.
    For example, ``auth_token`` maps to ``ROUTEFLOW_AUTH_TOKEN``.
    """

    model_config = SettingsConfigDict(
        env_prefix="ROUTEFLOW_",
        env_file=None if is_testing else ".env",
    ...
    port: int = 8000
    log_level: str = "INFO"
    database_path: str = "routeflow.db"
