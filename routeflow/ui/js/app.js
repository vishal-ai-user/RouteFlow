/* RouteFlow Server SPA Controller
 * Handles SPA routes, renders views, binds UI events, and tracks state.
 */

import { Auth, API } from './api.js?v=1.0.3';
import { Toast, Modal, Loader, EmptyState, renderPagination, ChartRenderer } from './components.js?v=1.0.3';
...
function renderLoginPage(container) {
        ...
        <div class="login-container">
            <div class="login-card">
                <div class="login-logo">
                    <span class="logo-mark" style="display: inline-flex; align-items: center; justify-content: center; margin-bottom: 8px; color: var(--accent-orange);"><svg viewBox="0 0 24 24" width="36" height="36" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12h4"></path><path d="M6 12c1.5-3 3.5-5 7-5h5"></path><polyline points="15 4 18 7 15 10"></polyline><path d="M6 12h12"></path><polyline points="15 9 18 12 15 15"></polyline><path d="M6 12c1.5 3 3.5 5 7 5h5"></path><polyline points="15 14 18 17 15 20"></polyline><circle cx="2" cy="12" r="1.5" fill="currentColor"></circle></svg></span>
                    <h1>RouteFlow Server</h1>
                    <p>Intelligent AI Gateway</p>
                </div>
                <form id="login-form">
                    <div class="form-group">
                        <label for="bearer-token">Bearer Auth Token</label>
                        <input type="password" id="bearer-token" class="form-control" placeholder="Enter ROUTEFLOW_AUTH_TOKEN" required autocomplete="current-password">
                    </div>
    ...
    form.onsubmit = async (e) => {
        ...
        try {
            ...
            Toast.success('Login Successful', 'Welcome to RouteFlow Server');
            window.location.hash = '#/dashboard';
        } catch (err) {
            Auth.clearToken();
    ...
    container.innerHTML = `
        ...
        <div class="dashboard-grid">
            <div class="stat-card">
                ...
                <span class="stat-footer">RouteFlow v${escapeHtml(summary?.server?.version || '0.1.0')}</span>
            </div>
    ...
    container.innerHTML = `
        <div class="actions-bar">
            <div class="search-box">
                <span class="search-icon" style="display: inline-flex; align-items: center; justify-content: center; margin-right: 8px; color: var(--text-secondary);"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg></span>
                <input type="text" id="provider-search" class="form-control search-control" placeholder="Search provider by name...">
            </div>
            <button id="btn-add-provider" class="btn btn-primary"><svg class="btn-icon" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg> Add Provider Key</button>
        </div>
        ...
        cards += `
            <div class="provider-card">
                ...
                <div class="provider-card-actions">
                    <button class="btn btn-secondary btn-sm btn-test-conn" data-id="${p.provider_id}" style="padding: 6px 12px; font-size: 11px;"><svg class="btn-icon" viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg> Test</button>
                    <button class="btn btn-secondary btn-sm btn-edit-provider" data-id="${p.provider_id}" style="padding: 6px 12px; font-size: 11px;"><svg class="btn-icon" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4Z"></path></svg> Edit</button>
                    <button class="btn btn-danger btn-sm btn-delete-provider" data-id="${p.provider_id}" style="padding: 6px 12px; font-size: 11px;"><svg class="btn-icon" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg> Delete</button>
...
async function renderMappingsPage(container) {
    container.innerHTML = `
        <div class="actions-bar">
            ...
            <button id="btn-add-mapping" class="btn btn-primary"><svg class="btn-icon" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg> Create Model Mapping</button>
        </div>
                ...
                trs += `
                    <tr>
                        ...
                        <td style="text-align: right;">
                            <button class="btn btn-secondary btn-sm btn-edit-mapping" data-logical="${m.logical_model}" data-nvidia="${m.nvidia_model}" style="padding: 6px 12px; font-size: 11px; margin-right: 8px;"><svg class="btn-icon" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4Z"></path></svg> Edit</button>
                            <button class="btn btn-danger btn-sm btn-delete-mapping" data-logical="${m.logical_model}" style="padding: 6px 12px; font-size: 11px;"><svg class="btn-icon" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg> Delete</button>
                        </td>
                    </tr>
                `;
            });
...
async function renderLogsPage(container) {
    ...
    container.innerHTML = `
        <div class="card">
            <div class="filters-row">
                ...
                <button id="btn-clear-logs" class="btn btn-danger" style="padding: 8px 14px; font-size: 13px; margin-left: auto;">Clear Logs</button>
            </div>
    ...
    if (clearBtn) {
        clearBtn.onclick = async () => {
            if (!confirm('Are you sure you want to permanently clear all logs? This will delete all request history, response logs, error logs, and token usage records.')) {
                return;
            }
            try {
                await API.clearAllLogs();
                Toast.show('All logs cleared successfully.', 'success');
                loadLogs(0);
            } catch (err) {
                Toast.show(`Failed to clear logs: ${err.message}`, 'error');
            }
        };
    }
...
async function renderErrorsPage(container) {
    ...
    container.innerHTML = `
        <div class="card">
            <div class="filters-row">
                ...
                <button id="btn-err-clear-logs" class="btn btn-danger" style="padding: 8px 14px; font-size: 13px; margin-left: auto;">Clear Logs</button>
            </div>
    ...
    const clearBtn = container.querySelector('#btn-err-clear-logs');
    if (clearBtn) {
        clearBtn.onclick = async () => {
            if (!confirm('Are you sure you want to permanently clear all logs? This will delete all request history, response logs, error logs, and token usage records.')) {
                return;
...
function initApp() {
    ...
    window.addEventListener('routeflow-unauthorized', () => {
        window.location.hash = '#/login';
    ...
    if (logoutBtn) {
        logoutBtn.onclick = () => {
            Modal.confirm(
                'Confirm Logout',
                'Do you wish to end your RouteFlow Server session?',
                async () => {
                    Auth.clearToken();
